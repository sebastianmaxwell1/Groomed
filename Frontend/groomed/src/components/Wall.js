...
class Wall extends React.Component {
    ...
    reloadPostReactions = (post) => {
        ...
        fetch(`http://localhost:8000/posts/${post._id}/reactions`, {
            method: 'GET',
            headers: {
            'Content-Type': 'application/json',
            'Authorization' : 'Bearer ' + localStorage.getItem("token")
    ...
    handle_newPost = (e, data) => {
        fetch('http://localhost:8000/posts/'+this.props.match.params.username, {
            method: 'POST'
            , headers: {
                'Content-Type': 'application/json',
                'Authorization' : 'Bearer ' + localStorage.getItem("token")
        ...
        return (
            ...
            <Grid centered columns={3}>
                <Grid.Row>
                    <Grid.Column>
                        <CreatePostForm />
                        {/* handle_newPost={this.handle_newPost} user={this.props.user} */}
                        <br></br>
                        <h2>Posts</h2>
                        {Posts ? Posts
                            :
